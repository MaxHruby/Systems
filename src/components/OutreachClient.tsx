...
export function OutreachClient({ leads, templates, signature, senderName, fromAddr, emailReady }: Props) {
  ...
  // Při změně leadu nastav příjemce a (je-li vybraná šablona) přerenderuj text.
  useEffect(() => {
    if (!selected) return;
  ...
  async function send() {
    ...
    if (res.ok) {
      setMsg({ kind: "ok", text: "Odesláno ✓ — zalogováno k leadu, fáze → Kontaktován, follow-up za 3 dny." });
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-slate-500">Vyber lead, šablonu, uprav a pošli — vše bez opuštění CRM.</p>
      ...
      {!emailReady && (
        <div className="card mb-4 border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          ...
          a nastav proměnné <code>SMTP_HOST=smtp.gmail.com</code>, <code>SMTP_PORT=465</code>, <code>SMTP_USER</code> (tvůj
          gmail), <code>SMTP_PASS</code> (app password), <code>SMTP_FROM_NAME</code>. Detailní postup je v README.
          Psát můžeš i teď — odeslání jen vyhodí chybu, dokud klíče nedoplníš.
        </div>
      )}
          ...
          <div className="max-h-[70vh] overflow-y-auto">
            {visible.length === 0 && (
              <p className="p-4 text-center text-sm text-slate-400">
                Žádné leady s emailem. Naimportuj firmy nebo doplň email v detailu.
              </p>
            )}
            {visible.map((l) => {
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Napiš zprávu… (placeholdery {jmeno}, {firma} se nahradily ze šablony)"
                />
              </div>

              {msg && (
                <p className={`rounded-lg px-3 py-2 text-sm ${msg.kind === "ok" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
                  {msg.text}
                </p>
              )}
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-slate-400">Odešle se z {fromAddr || "(nenapojeno)"}; odpovědi chodí do tvé schránky.</span>
              </div>
            </div>
          )}
        </div>
